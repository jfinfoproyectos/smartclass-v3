        ...
        <div className="flex items-center group w-full pr-2">
          ...
          {hasPage ? (
            <Link 
              ...
              className={cn(
                "flex-1 flex items-center gap-2 py-1.5 text-[10px] font-black uppercase tracking-[0.15em] transition-all truncate",
            ...
            <button 
              ...
              className="flex-1 flex items-center gap-2 py-1.5 text-[10px] font-black uppercase tracking-[0.15em] text-foreground/40 hover:text-primary transition-all text-left"
            >
        ...
        <AnimatePresence initial={false}>
          {isOpen && node.children && (
            <motion.div 
              ...
              className="ml-3 pl-3 border-l border-border/10 flex flex-col gap-0.5 mt-1 overflow-hidden"
            >
  ...
  return (
    <div className="px-0.5">
      <Link 
        ...
        className={cn(
          "flex items-center gap-2.5 px-3 py-2 w-full rounded-lg text-[13px] transition-all group mb-0.5 relative overflow-hidden",
          isActive 
            ? "text-primary font-bold bg-primary/5 ring-1 ring-primary/20 shadow-[inset_0_0_20px_rgba(var(--primary),0.02)]" 
            : "text-muted-foreground/70 hover:text-primary hover:bg-primary/5 font-medium",
      ...
      >
        ...
        {node.icon ? (
          <DynamicIcon icon={node.icon} className={cn("w-4 h-4 transition-transform group-hover:scale-110", isActive ? "text-primary" : "opacity-40 group-hover:opacity-100 group-hover:text-primary")} />
        ) : (
          <FileText className={cn("w-4 h-4 transition-transform group-hover:scale-110", isActive ? "text-primary" : "opacity-40 group-hover:opacity-100 group-hover:text-primary")} />
        )}
...
export function PublicSidebar({ navTree, projectId, className }: { 
  ...
  return (
    <aside className={cn("w-72 border-r border-border/40 h-full bg-card/30 backdrop-blur-md overflow-hidden flex flex-col", className)}>
      {/* Sidebar Toolbar */}
      <div className="p-4 py-2 border-b border-border/10 flex items-center justify-between bg-muted/20">
