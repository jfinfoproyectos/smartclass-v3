...
interface RouletteToolViewProps {
    courses: CourseWithStudents[];
}

export function RouletteToolView({ courses }: RouletteToolViewProps) {
    ...
    return (
        <div className="w-full flex-1 flex flex-col space-y-1.5 p-0.5 sm:p-1 min-h-0">
            {/* Barra Superior de la Ruleta: Solo Título y Selector de Ficha */}
            <div className="px-4 py-2 rounded-2xl border border-border bg-card text-card-foreground shadow-xs flex flex-wrap items-center justify-between gap-3 shrink-0">
                <div className="flex items-center gap-2">
                    <div className="p-1.5 rounded-xl bg-purple-500/10 text-purple-600 dark:text-purple-400 border border-purple-500/20 shrink-0">
                        <Dices className="h-4 w-4" />
                    </div>
                    <div>
                        <h2 className="text-sm font-bold text-foreground tracking-tight">Ruleta Aleatoria</h2>
                        <p className="text-[11px] text-muted-foreground hidden sm:block">
                            Sorteos de participación en vivo
                        </p>
                    </div>
                </div>
            ...
            {/* Contenido de la Ruleta adaptado exactamente al contenedor sin scroll */}
                ...
                {currentStudents.length === 0 ? (
                    <div className="p-8 text-center rounded-2xl border border-dashed border-border bg-card/60 space-y-2 my-auto">
                        ...
                        <p className="text-xs text-muted-foreground max-w-md mx-auto">
                            La ficha seleccionada no cuenta con estudiantes inscritos actualmente. Selecciona otra ficha activa desde el selector superior para comenzar a girar la ruleta.
                        </p>
                    </div>
                ) : (
