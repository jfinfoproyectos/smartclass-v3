...
import { getPostLogoutRedirect, signOut, changeUserPassword, setUserPassword } from "@/features/auth/services/authService"
...
export function NavUser({
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (typeof (profile as any)?.hasPassword === "boolean") {
        setHasPassword((profile as any).hasPassword)
      }
    } catch (error) {
  ...
  const handleChangePassword = async () => {
    ...
    if (!newPassword) {
      setPasswordError("Debes ingresar la nueva contraseña.")
      return
    }
  ...
  return (
    <>
      ...
      <Dialog open={accountOpen} onOpenChange={setAccountOpen}>
        <DialogContent
          className="sm:max-w-xl md:max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl"
        ...
        >
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Mi Cuenta & Perfil</DialogTitle>
            <DialogDescription className="text-xs">
              Gestiona tus datos personales, de contacto y credenciales de seguridad.
            </DialogDescription>
          </DialogHeader>

          <Tabs defaultValue="profile" className="w-full mt-2">
            <TabsList className="grid w-full grid-cols-2 rounded-2xl p-1 bg-slate-100 dark:bg-slate-800/80 mb-2">
              <TabsTrigger value="profile" className="rounded-xl text-xs font-semibold gap-2 py-2">
                <User className="w-3.5 h-3.5" /> Datos Personales
              </TabsTrigger>
              <TabsTrigger value="security" className="rounded-xl text-xs font-semibold gap-2 py-2">
                <KeyRound className="w-3.5 h-3.5" /> Seguridad y Contraseña
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="security" className="space-y-4 pt-1">
              {!hasPassword && (
                <div className="p-3.5 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs leading-relaxed flex items-start gap-2.5">
                  <ShieldCheck className="w-5 h-5 shrink-0 text-blue-400 mt-0.5" />
                  <div>
                    <span className="font-semibold block">Cuenta con inicio de sesión social</span>
                    Actualmente ingresas con Google. Puedes definir una contraseña a continuación para tener la opción de acceder también con tu correo y contraseña.
                  </div>
                </div>
              )}
              ...
              {hasPassword && (
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <button
                      type="button"
                      className="text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 absolute inset-y-0 end-0 flex h-full w-10 items-center justify-center transition-colors outline-none"
              ...
              {hasPassword && (
                <div className="flex items-center space-x-2 pt-1">
                  ...
                  >
                    Cerrar sesión en otros dispositivos tras el cambio
                  </label>
                </div>
              )}
              ...
              <DialogFooter className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                <Button variant="outline" className="rounded-xl text-xs font-bold" onClick={() => setAccountOpen(false)}>
                  Cerrar
                </Button>
                <Button
                  className="rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs shadow-md shadow-emerald-500/20"
                  onClick={handleChangePassword}
                  disabled={changingPassword || !newPassword || !confirmPassword || (hasPassword && !currentPassword)}
                >
                  {changingPassword ? "Actualizando..." : hasPassword ? "Actualizar Contraseña" : "Establecer Contraseña"}
                </Button>
              </DialogFooter>
            </TabsContent>
          </Tabs>
